...
export function Header() {
  ...
  return <header className="site-header">
    <Link className="corner-logo" href="/" scroll={true} aria-label="מקיף ז׳ אשדוד, עמוד הבית"><img src="/makif-z-logo.png" alt="מקיף ז׳ אשדוד"/></Link>
    <div className="nav-shell"><span className="header-logo-spacer" aria-hidden="true"/><nav className="desktop-nav" aria-label="ניווט ראשי">{links.map(([l,h])=><Link key={h} href={h} scroll={!h.includes("#")}>{l}</Link>)}</nav><button className="menu-button" onClick={()=>setOpen(!open)} aria-expanded={open} aria-label={open?"סגירת תפריט":"פתיחת תפריט"}>{open?<X/>:<Menu/>}</button></div>
    {open&&<nav className="mobile-nav" aria-label="ניווט במובייל">{links.map(([l,h])=><Link key={h} href={h} scroll={!h.includes("#")} onClick={()=>setOpen(false)}>{l}</Link>)}</nav>}
